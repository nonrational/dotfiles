...
function validateData(data) {
  if (data.skill !== 'comment-register') {
    throw new Error(`Expected skill "comment-register", got ${JSON.stringify(data.skill)}`);
  }
...
function buildDiscriminationPrompt(item) {
  ...
  const prompt = `Review these inline source comments. Use the comment-register skill if it is available.
...
function buildTransformationPrompt(item) {
  return `Edit inline source comments. Use the comment-register skill if it is available.
...
function buildDetectionPrompt(item) {
  return `Review inline source comments. Use the comment-register skill if it is available.
...
async function main() {
  ...
  if (opts.validate) {
    console.log(`Validated ${validation.caseCount} comment-register eval cases.`);
    return;
  }
