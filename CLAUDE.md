...
This repo is also consumed downstream by the user's agentic alter-ego, @nonreagent, running unattended on its own host(s). It picks up `home/.claude` changes (hooks, settings.json) as soon as it next pulls and redeploys — not necessarily in lockstep with this checkout — so weigh that lag when changing anything under `home/.claude` (see Gotchas).
...
- `make eval SKILL=<name>` — full keyed run of one skill's `evals/` suite (one Markdown case per file, `evals/README.md` for the suite; see `docs/superpowers/specs/2026-09-25-eval-cases-markdown-design.md`) through promptfoo, gated by `evals/bin/check-gate.mjs`; spends subscription usage. `make eval-compare SKILL=<name>` runs skill vs baseline side by side. Unset `ANTHROPIC_API_KEY` first, or `claude -p` bills the API instead.
- `make eval-local SKILL=<name>` — runs the same suite against a local Ollama model (default `gemma4:latest`, override `EVAL_LOCAL_MODEL`), free, about a minute per case. It puts the skill in the system prompt under `--bare`, so it checks that cases render and grade, not how the skill loads. Sonnet runs (`make eval`) stay the reference.
...
- **New hooks must degrade silently on a lagging downstream deploy.** @nonreagent's hosts redeploy this repo independently of this checkout, so a hook added here can run there before that host has pulled it. When the referenced script doesn't exist yet, the shell fails with a `not found` error before the script's own error handling can run. Non-blocking Stop/Notification hooks failing this way is cosmetic, not a break, but keep new hooks additive and self-contained so a stale copy elsewhere no-ops rather than erroring loudly.
