...
- **New hooks must degrade silently on a lagging downstream deploy.** @nonreagent's hosts redeploy this repo independently of this checkout, so a hook added here can run there before that host has pulled it. When the referenced script doesn't exist yet, the shell fails with a `not found` error *before* the script's own error handling ever gets a chance to run — the script's own graceful-exit logic (see `op-sync-usage.sh` for the pattern: default to `exit 0`, only surface detail behind a `-v` flag) can't protect against that. Non-blocking Stop/Notification hooks failing this way is cosmetic, not a break, but keep new hooks additive and self-contained so a stale copy elsewhere just no-ops rather than erroring loudly.
